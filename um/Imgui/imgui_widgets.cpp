...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    // disabled background
    window->DrawList->AddRectFilled(rect.Min, rect.Min + ImVec2(20, 20), ImColor(33, 35, 38), 4.f);

    // active background
    window->DrawList->AddRectFilled(rect.Min, rect.Min + ImVec2(20, 20), ImColor(30, 46, 55, it_anim->second.active_alpha), 4.f);

    // middle circle
    window->DrawList->AddCircleFilled(rect.Min + ImVec2(10, 10), 4, ImColor(61, 155, 204, it_anim->second.active_alpha));

    // text
    window->DrawList->AddText(rect.Min + ImVec2(26, 2), ImColor(255, 255, 255, 45 + it_anim->second.active_alpha), label);

    return pressed;
}

struct toggle_checkbox {
    float animation;
};

bool ImGui::CustomCheckbox(const char* label, const char* description, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::map <ImGuiID, toggle_checkbox> animate;
    ...
    ImU32 col_bg;
    col_bg = ImGui::GetColorU32(ImVec4(0.50f, 0.47f, 0.94f, it_anim->second.animation));

    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(37, 37, 37), false, 9.0f);
    ...
    // description text
    ImGui::PushFont(details);
    window->DrawList->AddText(rect.Min + ImVec2(0, 25), ImColor(63, 65, 68), description);
    ImGui::PopFont();
...
bool ImGui::Tab(const char* label, bool selected)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(label_size.x + 15, 40), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    if (selected)
    {
        window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Max.x + 11, bb.Max.y + 10), ImColor(52, 55, 59), 6.f);
        ImGui::RenderText(ImVec2(bb.Min.x + size.x / 2 - (label_size.x / 2) + 6, bb.Min.y + (size.y / 2) - (label_size.y / 2) + 5), label);
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y + 9), ImVec2(bb.Min.x + 3, bb.Max.y), ImColor(128, 121, 239));
    }
    else
    {
        ImGui::RenderText(ImVec2(bb.Min.x + size.x / 2 - (label_size.x / 2) + 6, bb.Min.y + (size.y / 2) - (label_size.y / 2) + 5), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float size = GetWindowWidth() - 40;
    ...
    ItemSize(rect, style.FramePadding.y);
    if (!ItemAdd(clickable, id, &frame_bb))
        return false;
    ...
    it_anim->second.arrow_anim = ImLerp(it_anim->second.arrow_anim, popup_open ? 0.8f : 0.3f, 0.05f * (1.0f - ImGui::GetIO().DeltaTime));
    ...
    window->DrawList->AddText(ImVec2(rect.Min.x + 2, rect.Min.y), GetColorU32(ImGuiCol_Text), label);

    ImGui::PushFont(arrow);
    window->DrawList->AddText(ImVec2(rect.Max.x - 25, rect.Max.y - 25), ImColor(1.f, 1.f, 1.f, it_anim->second.arrow_anim), "G");
    ImGui::PopFont();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImRect frame_bb(total_bb.Min + ImVec2(80, label_size.y + 12), total_bb.Max);
    ...
    // background
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(33, 35, 38), 5.0f);
    // active
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(128, 121, 239), 5.0f);
    // circle
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 7, grab_bb.Min.y), 6.0f, ImColor(128, 121, 239), 30);

    // label
    RenderText(ImVec2(total_bb.Min.x, total_bb.Min.y + 1), label);

    // value
    RenderTextClipped(total_bb.Min, total_bb.Max, value_buf, value_buf_end, NULL, ImVec2(1.0, 0.15));

    return value_changed;
}

bool ImGui::CustomSliderScalar(const char* label, const char* description, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.25f, 0.25f, 0.27f, 1.00f));
    //description
    ImGui::PushFont(details);
    RenderText(ImVec2(total_bb.Min.x, total_bb.Min.y + 23), description);
    ImGui::PopFont();
...
bool ImGui::CustomSliderFloat(const char* label, const char* description, float* v, float v_min, float v_max, const char* format, float power)
{
    return CustomSliderScalar(label, description, ImGuiDataType_Float, v, &v_min, &v_max, format, power);
}
...
bool ImGui::CustomSliderInt(const char* label, const char* description, int* v, int v_min, int v_max, const char* format, float power)
{
    return CustomSliderScalar(label, description, ImGuiDataType_S32, v, &v_min, &v_max, format, power);
}
...
bool ImGui::CustomColorEdit4(const char* label, const char* description, float col[4], ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2(250, 0);
    bool value_changed = false;
    bool value_changed_as_float = false;
    ...
    {
        if (!(flags & ImGuiColorEditFlags_NoInputs))
            SameLine(250, style.ItemInnerSpacing.x);

        const ImVec4 col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(25,15)))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                // Store current color and open a picker
    ...
    // description text
    ImGui::PushFont(details);
    window->DrawList->AddText(pos - ImVec2(250, -22), ImColor(63, 65, 68), description);
    ImGui::PopFont();
